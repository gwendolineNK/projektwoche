...
public class Spielfeld {
    ...
    public boolean istEinfacherZugGueltig(int z1, int s1, int z2, int s2, int nummer) {
        // TODO 9
    	if(!imFeld(z1,s1) && !imFeld(z2,s2)) {
    		return false;
    	}
    	if(getZelle(z1,s1) != nummer && getZelle(z2,s2) != 0) {
    		return false;
    	}
    	if(z2 - z1 != richtung(nummer) && (s2 - s1 != 1 || s2 - s1 != -1)) {
    		return false;
    	}
        return true;
    }
    ...
    public boolean istSchlagzugGueltig(int z1, int s1, int z2, int s2, int nummer) {
    	...
    	if(z2 - z1 != 2*richtung(nummer) && (s2 - s1 != 2 || s2 - s1 != -2)) {
    		return false;
    	}
    	
    	int zm = (z1 + z2) / 2;
    	int sm = (s1 + s2) / 2;
    	if(feld[zm][sm] != gegner(nummer)) {
    		return false;
    	}
        return true;
    }
    ...
    public boolean hatGewonnen(int nummer) {
        // TODO 12
        return zaehleSteine(gegner(nummer)) == 0;
    }
